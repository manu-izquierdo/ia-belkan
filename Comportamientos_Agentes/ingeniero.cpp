...
list<Action> ComportamientoIngeniero::B_Anchura(const EstadoI &inicio, const EstadoI &final, const vector<vector<unsigned char>> &terreno, const vector<vector<unsigned char>> &altura) {
  ...
  while (!frontera.empty()) {
    ...
    bool tiene_jump = nodo_actual.estado.jump;
    if (visitados[nodo_actual.estado.site.f][nodo_actual.estado.site.c][nodo_actual.estado.site.brujula][zapatillas][tiene_jump])
      continue; // Si ya se ha visitado, no se vuelve a visitar
    visitados[nodo_actual.estado.site.f][nodo_actual.estado.site.c][nodo_actual.estado.site.brujula][zapatillas][tiene_jump] = true; // Marcamos el estado como explorado
    ...
    if (nodo_actual.estado.site.f == final.site.f && nodo_actual.estado.site.c == final.site.c) {
      if(tiene_jump){
        cout << "He encontrado una secuencia que tiene al menos un jump" << endl;
